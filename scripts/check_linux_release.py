...
APP_ID = "io.github.abigordiy.open-trofeo-lcd"
...
def iter_asset_refs(value: object) -> list[str]:
    refs: list[str] = []
    if isinstance(value, dict):
        for nested in value.values():
            refs.extend(iter_asset_refs(nested))
        ...
        if "/" in text and suffix in ASSET_EXTENSIONS:
            refs.append(text)
    return refs


def install_rule_covers(path_text: str, sources: list[str]) -> bool:
    for source in sources:
        source = source.rstrip("/")
        if not source:
            continue
        if any(char in source for char in "*?["):
            if fnmatch(path_text, source):
                return True
            continue
...
def main() -> int:
    ...
    if deb_install.exists():
        ...
        for path_text in sorted(EXPECTED_THEMES):
            theme_path = ROOT / path_text
            ...
            try:
                ...
                continue
            for asset_ref in sorted(set(iter_asset_refs(theme_data))):
                repo_asset_path = f"themes/{asset_ref}"
                require((ROOT / repo_asset_path).exists(), f"{path_text}: referenced asset is missing: {repo_asset_path}")
                require(
                    install_rule_covers(repo_asset_path, install_sources),
                    f"DEB install does not include theme asset referenced by {path_text}: {repo_asset_path}",
                )
