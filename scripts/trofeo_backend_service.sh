...
UNIT="trofeo-backend.service"
LOG_FILE="${HOME}/.local/state/open-trofeo-lcd/backend.log"
...
case "${cmd}" in
  install)
    "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/install_backend_service.sh"
    ;;
  start)
    systemctl --user stop trofeo-lcd.service >/dev/null 2>&1 || true
