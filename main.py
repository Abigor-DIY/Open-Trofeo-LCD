...
SHOW_REQUEST_FILE = STATE_DIR / "show-window.request"
...
def _request_existing_window() -> None:
    try:
        STATE_DIR.mkdir(parents=True, exist_ok=True)
        SHOW_REQUEST_FILE.write_text(f"{time.time():.6f}\n", encoding="utf-8")
    except Exception:
        pass
...
def _acquire_instance_lock():
    ...
    try:
        ...
        if pid and not _pid_is_alive(pid) and not cmdline:
            print(f"[+] Czyszczę martwy lock launchera (PID: {pid}).")
            lock_handle.close()
            try:
        ...
        if pid:
            ...
            print("[+] Wysłano żądanie pokazania istniejącego okna GUI.")
        else:
