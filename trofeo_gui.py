...
APP_STATE_DIR = Path.home() / ".local/state/open-trofeo-lcd"
SHOW_REQUEST_PATH = APP_STATE_DIR / "show-window.request"
...
class TrofeoGui(QMainWindow):
    ...
    def __init__(self, base_url: str):
        ...
        try:
            self._show_request_mtime_ns = SHOW_REQUEST_PATH.stat().st_mtime_ns
        except OSError:
            self._show_request_mtime_ns = 0
        ...
        self._lcd_config_restored_to_backend = False
        ...
        QTimer.singleShot(900, self._apply_restored_lcd_config_to_backend)
        ...
        self.show_request_timer.timeout.connect(self._poll_show_request)
        self.show_request_timer.start(700)
    ...
    def _humanize_stat_source(self, source: str) -> str:
        custom = {
            ...
            "game_fps": "Game: FPS",
            "game_frametime_ms": "Game: Frametime",
            "game_fps_source": "Game: FPS Source",
    ...
    def _designer_source_groups(self, domain: str = "all") -> list[tuple[str, list[str]]]:
        ...
        system_groups = [
            ...
            ("Gaming", ["game_active", "game_name", "game_launcher", "game_fps", "game_frametime_ms", "game_overlay"]),
    ...
    def _apply_configuration_preferences(self) -> None:
        ...
        if hasattr(self, "cfg_brightness_slider"):
            self.apply_lcd_config()
        self._save_ui_state()
        self.append_log("[config] Zastosowano ustawienia interfejsu.")

    def _lcd_config_payload(self) -> dict[str, object]:
        slider = getattr(self, "cfg_brightness_slider", None)
        brightness = int(slider.value()) if slider is not None else 100
        return {"brightness_percent": max(0, min(150, brightness))}

    def _update_lcd_brightness_label(self, value: int | None = None) -> None:
        if value is None and hasattr(self, "cfg_brightness_slider"):
            value = int(self.cfg_brightness_slider.value())
        if hasattr(self, "cfg_brightness_value_label"):
            self.cfg_brightness_value_label.setText(f"{int(value or 0)}%")

    def apply_lcd_config(self) -> None:
        if not hasattr(self, "cfg_brightness_slider"):
            return
        self.api_call("config", "POST", "/v1/config", self._lcd_config_payload())
        self._lcd_config_restored_to_backend = True
        self._save_ui_state()
        self.append_log(f"[lcd] Applied brightness: {int(self.cfg_brightness_slider.value())}%.")

    def _apply_restored_lcd_config_to_backend(self) -> None:
        if self._lcd_config_restored_to_backend or not hasattr(self, "cfg_brightness_slider"):
            return
        self._lcd_config_restored_to_backend = True
        self.api_call("config", "POST", "/v1/config", self._lcd_config_payload())
        self.append_log("[lcd] Restored saved LCD brightness.")
    ...
    def _reset_configuration_defaults(self) -> None:
        ...
        if hasattr(self, "cfg_brightness_slider"):
            self.cfg_brightness_slider.setValue(100)
    ...
    def _build_ui(self, base_url: str) -> None:
        ...
        recovery_box = QGroupBox("LCD USB recovery")
        recovery_layout = QHBoxLayout(recovery_box)
        recovery_layout.setContentsMargins(16, 18, 16, 14)
        recovery_layout.setSpacing(10)
        self.usb_recovery_hint_label = QLabel(
            "Use after deep suspend, USB timeout or frozen clock/EQ. Requires sudo and briefly resets the Trofeo LCD USB device. "
            "A full PC reboot usually helps too; if USB remains powered in standby, power off the PC for a few seconds."
        )
        self.usb_recovery_hint_label.setWordWrap(True)
        self.usb_recovery_hint_label.setStyleSheet("color: #a8b3c7;")
        self.usb_recovery_btn = QPushButton("Run USB recovery...")
        self.usb_recovery_btn.setToolTip(
            "Runs scripts/trofeo_usb_power_cycle.sh from a terminal with sudo, then restarts the backend."
        )
        self.usb_recovery_btn.clicked.connect(self.run_usb_recovery)
        recovery_layout.addWidget(self.usb_recovery_hint_label, 1)
        recovery_layout.addWidget(self.usb_recovery_btn)
        runtime_device_layout.addWidget(recovery_box)
        ...
        self.cfg_brightness_slider.setRange(0, 150)
        self.cfg_brightness_slider.setValue(100)
        self.cfg_brightness_slider.setToolTip(
            "Scales the rendered LCD output brightness. 100% keeps original theme colors."
        )
        self.cfg_brightness_value_label = QLabel("100%")
        self.cfg_brightness_value_label.setMinimumWidth(48)
        self.cfg_brightness_value_label.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
        self.cfg_brightness_slider.valueChanged.connect(self._update_lcd_brightness_label)
        self.cfg_brightness_slider.sliderReleased.connect(self.apply_lcd_config)
        brightness_row = QHBoxLayout()
        brightness_row.setContentsMargins(0, 0, 0, 0)
        brightness_row.setSpacing(8)
        brightness_row.addWidget(self.cfg_brightness_slider, 1)
        brightness_row.addWidget(self.cfg_brightness_value_label)
        ...
        lcd_form.addRow("LCD brightness:", brightness_widget)
        ...
        self.cfg_usb_recovery_btn = QPushButton("LCD USB Recovery (sudo)")
        self.cfg_usb_recovery_note_label = QLabel(
            "For a frozen LCD after deep sleep: run USB recovery from here or from a terminal. "
            "If it still shows an old frame, reboot usually helps; full power-off is stronger than a warm reboot."
        )
        self.cfg_usb_recovery_note_label.setWordWrap(True)
        self.cfg_usb_recovery_note_label.setObjectName("selectionSummaryLabel")
        ...
        self.cfg_usb_recovery_btn.clicked.connect(self.run_usb_recovery)
        for button in (
            self.cfg_usb_recovery_btn,
            self.cfg_reset_btn,
            self.cfg_export_btn,
            self.cfg_import_btn,
        ...
        quick_cfg_layout.addWidget(self.cfg_usb_recovery_note_label)
        ...
        self.designer_auto_layout_btn = AnimatedToolbarButton("Arrange")
        ...
        self.designer_auto_layout_btn.setMinimumWidth(72)
        self.designer_auto_layout_btn.setObjectName("secondaryAccentButton")
        ...
        self.designer_auto_layout_btn.setToolTip(
            self._tr(
                "Auto-arrange selected layers, or all visible widgets and stats when nothing is selected.",
                "Automatycznie rozmieść zaznaczenie albo wszystkie widoczne widgety i statystyki, gdy nic nie jest zaznaczone.",
            )
        )
        ...
        preview_tools_row.addWidget(self.designer_auto_layout_btn)
        preview_tools_row.addSpacing(6)
        ...
        self.designer_auto_layout_btn.clicked.connect(self.auto_arrange_designer_elements)
    ...
    def _poll_show_request(self) -> None:
        try:
            stat = SHOW_REQUEST_PATH.stat()
        except OSError:
            return
        mtime_ns = stat.st_mtime_ns
        if mtime_ns == self._show_request_mtime_ns:
            return
        self._show_request_mtime_ns = mtime_ns
        self.show_from_tray()
    ...
    def hide_to_tray(self) -> None:
        ...
        if self._tray_icon is not None and not self._tray_message_shown:
            self._tray_icon.showMessage(
                "Open Trofeo LCD",
                "Aplikacja działa w trayu. Kliknij ikonę albo uruchom aplikację z menu ponownie, aby przywrócić okno.",
    ...
    def closeEvent(self, event) -> None:  # type: ignore[override]
        if self._close_to_tray_enabled and self._tray_available():
            event.ignore()
            self.hide_to_tray()
            return
    ...
    def _auto_layout_kind(self, collection: str, item: dict[str, Any]) -> str:
        if collection == "widgets":
            kind = str(item.get("kind", "")).strip().lower()
            if kind == "media_now_playing":
                return "media"
        ...
        if collection == "stats":
            ...
            if display == "gauge":
                return "gauge"
            if display == "equalizer" or source.startswith("audio_eq"):
                return "eq"
            if display in {"progress", "sparkline"}:
                return display
            return "stat"
        if collection == "texts":
            return "text"
    ...
    def _auto_layout_target_entries(self) -> tuple[list[tuple[str, int, dict[str, Any]]], bool]:
        selected = [
            (collection, row, item)
            for collection, row, item in self._selected_items_multi_any()
            if isinstance(item, dict) and not bool(item.get("locked", False))
        ]
        if selected:
            return selected, True
        entries: list[tuple[str, int, dict[str, Any]]] = []
        for collection in ("widgets", "stats"):
            for row, item in enumerate(self._theme_items_for_collection(collection)):
                if isinstance(item, dict) and bool(item.get("visible", True)) and not bool(item.get("locked", False)):
                    entries.append((collection, row, item))
        for row, item in enumerate(self._theme_items_for_collection("images")):
            if not isinstance(item, dict) or not bool(item.get("visible", True)) or bool(item.get("locked", False)):
                continue
            if self._auto_layout_kind("images", item) in {"clock", "media_image"}:
                entries.append(("images", row, item))
        return entries, False

    def _auto_layout_set_rect(
        self,
        collection: str,
        item: dict[str, Any],
        x: int,
        ...
        *,
        ...
        width = max(24, min(int(width), int(canvas_width)))
        height = max(18, min(int(height), int(canvas_height)))
        x = max(0, min(int(x), int(canvas_width) - width))
        y = max(0, min(int(y), int(canvas_height) - height))
        if collection in {"images", "panels", "widgets"}:
            item["rect"] = [self._snap_value(x), self._snap_value(y), self._snap_value(width), self._snap_value(height)]
        else:
            item["x"] = self._snap_value(x)
            item["y"] = self._snap_value(y)
            item["box_width"] = self._snap_value(width)
            item["box_height"] = self._snap_value(height)
            if align is not None:
                item["align"] = align

    def auto_arrange_designer_elements(self) -> None:
        if self.theme_doc_model is None:
            self.reload_designer_from_json()
        if self.theme_doc_model is None:
            return
        entries, selection_only = self._auto_layout_target_entries()
        if not entries:
            QMessageBox.information(
                self,
                self._tr("Auto arrange", "Automatyczny układ"),
                self._tr(
                    "Select layers to arrange, or add visible widgets/stats first.",
                    "Zaznacz warstwy do rozmieszczenia albo dodaj najpierw widoczne widgety/statystyki.",
                ),
            )
            return

        canvas = self.theme_doc_model.get("canvas", {}) if isinstance(self.theme_doc_model, dict) else {}
        canvas_width = int(canvas.get("width", 1920) or 1920)
        canvas_height = int(canvas.get("height", 462) or 462)
        margin_x = max(32, int(round(canvas_width * 0.026)))
        margin_y = max(18, int(round(canvas_height * 0.045)))
        gap = max(12, int(round(canvas_width * 0.010)))
        usable_w = max(1, canvas_width - (2 * margin_x))
        left_w = min(560, max(360, int(usable_w * 0.30)))
        right_w = min(500, max(360, int(usable_w * 0.25)))
        center_x = margin_x + left_w + gap
        right_x = canvas_width - margin_x - right_w
        center_w = max(420, right_x - gap - center_x)
        bottom_y = max(margin_y, canvas_height - margin_y - 82)
        ...
        for entry in entries:
            kind = self._auto_layout_kind(entry[0], entry[2])
            buckets.setdefault(kind, []).append(entry)

        self.push_designer_history()

        # Left telemetry cluster: gauges get the most predictable space.
        gauges = buckets.get("gauge", [])
        if gauges:
            gauge_cols = min(3, max(1, len(gauges)))
            gauge_rows = (len(gauges) + gauge_cols - 1) // gauge_cols
            gauge_size = min(154, max(96, int((left_w - gap * (gauge_cols - 1)) / gauge_cols)))
            if gauge_rows > 1:
                gauge_size = min(gauge_size, max(88, int((canvas_height - 2 * margin_y - gap * (gauge_rows - 1)) / gauge_rows)))
            start_x = margin_x + max(0, (left_w - (gauge_cols * gauge_size + (gauge_cols - 1) * gap)) // 2)
            start_y = margin_y + max(8, int((canvas_height - 2 * margin_y - (gauge_rows * gauge_size + (gauge_rows - 1) * gap)) * 0.40))
            for idx, (collection, _row, item) in enumerate(gauges):
                col = idx % gauge_cols
                row = idx // gauge_cols
                self._auto_layout_set_rect(
                    collection,
                    item,
                    start_x + col * (gauge_size + gap),
                    start_y + row * (gauge_size + gap),
                    gauge_size,
                    gauge_size,
                    canvas_width,
                    canvas_height,
        ...
        # Center: now playing first, then progress/sparkline rows.
        media_items = buckets.get("media", [])
        media_h = 124 if buckets.get("progress") or buckets.get("sparkline") or buckets.get("eq") or buckets.get("stat") else 148
        for idx, (collection, _row, item) in enumerate(media_items):
            item_h = media_h if idx == 0 else 92
            item_y = margin_y + 46 + idx * (item_h + 10)
            item_w = min(center_w, 860 if idx == 0 else 620)
            self._auto_layout_set_rect(
                collection,
                item,
                center_x + max(0, (center_w - item_w) // 2),
                item_y,
                item_w,
                item_h,
                canvas_width,
        ...
        flow_items = buckets.get("progress", []) + buckets.get("sparkline", []) + buckets.get("stat", []) + buckets.get("widget", [])
        flow_start_y = margin_y + 206 if media_items else margin_y + 72
        flow_cols = 2 if len(flow_items) > 1 and center_w >= 620 else 1
        flow_w = int((center_w - gap * (flow_cols - 1)) / flow_cols)
        for idx, (collection, _row, item) in enumerate(flow_items):
            kind = self._auto_layout_kind(collection, item)
            item_h = 74 if kind == "sparkline" else 42
            col = idx % flow_cols
            row = idx // flow_cols
            self._auto_layout_set_rect(
                collection,
                item,
                center_x + col * (flow_w + gap),
                flow_start_y + row * (item_h + 10),
                flow_w,
                item_h,
                canvas_width,
                canvas_height,
        ...
        # Right: weather, forecast, clock and EQ keep consistent readable sizes.
        weather_items = buckets.get("weather", [])
        forecast_items = buckets.get("forecast", [])
        clock_items = buckets.get("clock", [])
        eq_items = buckets.get("eq", [])
        for idx, (collection, _row, item) in enumerate(weather_items):
            self._auto_layout_set_rect(
                collection,
                item,
                right_x,
        ...
        for idx, (collection, _row, item) in enumerate(forecast_items):
            y = margin_y + 188 + idx * 118 if weather_items else margin_y + 76 + idx * 118
            self._auto_layout_set_rect(
                collection,
                item,
                right_x,
        ...
        for idx, (collection, _row, item) in enumerate(clock_items):
            size = min(136, max(96, right_w // 3))
            self._auto_layout_set_rect(
                collection,
                item,
                right_x + right_w - size,
                margin_y + idx * (size + 8),
                size,
                size,
                canvas_width,
        ...
        for idx, (collection, _row, item) in enumerate(eq_items):
            eq_w = min(right_w, 360)
            self._auto_layout_set_rect(
                collection,
                item,
                right_x + right_w - eq_w,
                bottom_y - idx * 82,
                eq_w,
                72,
                canvas_width,
        ...
        # If the user explicitly selected text/image layers, place leftovers in a tidy bottom row.
        leftovers = buckets.get("text", []) + buckets.get("image", []) + buckets.get("media_image", [])
        if leftovers:
            item_w = max(180, min(360, int(usable_w / max(1, len(leftovers)))))
            for idx, (collection, _row, item) in enumerate(leftovers):
                x = margin_x + idx * (item_w + 8)
                h = 44 if collection == "texts" else 96
                self._auto_layout_set_rect(
                    collection,
                    item,
                    x,
                    canvas_height - margin_y - h,
                    item_w,
                    h,
                    canvas_width,
                    canvas_height,
        ...
        self.preview_info_label.setText(
            self._tr(
                f"Auto-arranged {len(entries)} selected layer(s)." if selection_only else f"Auto-arranged {len(entries)} visible widget/stat layer(s).",
                f"Automatycznie rozmieszczono {len(entries)} zaznaczonych warstw." if selection_only else f"Automatycznie rozmieszczono {len(entries)} widocznych widgetów/statystyk.",
            )
        )
    ...
    def _setup_designer_layers_panel(self, parent_layout: QVBoxLayout) -> None:
        ...
        self.quick_add_game_fps_btn = QPushButton("Game FPS")
        self.quick_add_game_fps_btn.setObjectName("quickAddButton")
        self.quick_add_game_fps_btn.setMinimumHeight(24)
        ...
        widgets_grid.addWidget(self.quick_add_game_fps_btn, 3, 0, 1, 2)
        ...
        self.quick_add_game_fps_btn.clicked.connect(self.add_game_fps_widget)
    ...
    def _refresh_extended_ui_labels(self) -> None:
        ...
        if hasattr(self, "quick_add_game_fps_btn"):
            self.quick_add_game_fps_btn.setText(tr("Game FPS", "FPS gry"))
        ...
        if hasattr(self, "designer_auto_layout_btn"):
            self.designer_auto_layout_btn.setText(tr("Arrange", "Ułóż"))
            self.designer_auto_layout_btn.setToolTip(
                tr(
                    "Auto-arrange selected layers, or all visible widgets and stats when nothing is selected.",
                    "Automatycznie rozmieść zaznaczenie albo wszystkie widoczne widgety i statystyki, gdy nic nie jest zaznaczone.",
    ...
    def _refresh_localized_texts(self) -> None:
        ...
        if hasattr(self, "cfg_usb_recovery_btn"):
            self.cfg_usb_recovery_btn.setText(self._tr("LCD USB Recovery (sudo)", "Recovery USB LCD (sudo)"))
        if hasattr(self, "cfg_usb_recovery_note_label"):
            self.cfg_usb_recovery_note_label.setText(
                self._tr(
                    "For a frozen LCD after deep sleep: run USB recovery from here or from a terminal. "
                    "If it still shows an old frame, reboot usually helps; full power-off is stronger than a warm reboot.",
                    "Gdy LCD zamarznie po głębokim uśpieniu: uruchom recovery USB tutaj albo z terminala. "
                    "Jeżeli nadal pokazuje starą klatkę, reboot zwykle pomaga; pełne wyłączenie zasilania jest mocniejsze niż ciepły restart.",
                )
            )
        if hasattr(self, "usb_recovery_btn"):
            self.usb_recovery_btn.setText(self._tr("Run USB recovery...", "Uruchom recovery USB..."))
        if hasattr(self, "usb_recovery_hint_label"):
            self.usb_recovery_hint_label.setText(
                self._tr(
                    "Use after deep suspend, USB timeout or frozen clock/EQ. Requires sudo and briefly resets the Trofeo LCD USB device. "
                    "A full PC reboot usually helps too; if USB remains powered in standby, power off the PC for a few seconds.",
                    "Użyj po głębokim uśpieniu, timeoutach USB albo zamrożonym zegarze/EQ. Wymaga sudo i krótko resetuje urządzenie USB Trofeo LCD. "
                    "Pełny reboot PC zwykle też pomaga; jeżeli USB zostaje zasilone w standby, wyłącz PC na kilka sekund.",
                )
            )
    ...
    def _restore_ui_state(self) -> None:
        ...
        try:
            ...
            if hasattr(self, "cfg_brightness_slider"):
                try:
                    self.cfg_brightness_slider.setValue(max(0, min(150, int(payload.get("brightness_percent", 100)))))
                except Exception:
                    self.cfg_brightness_slider.setValue(100)
                self._update_lcd_brightness_label()
            if hasattr(self, "cfg_weather_lat_edit"):
    ...
    def _save_ui_state(self, *, onboarding_done: bool | None = None) -> None:
        ...
        if hasattr(self, "cfg_brightness_slider"):
            payload["brightness_percent"] = int(self.cfg_brightness_slider.value())
        if hasattr(self, "cfg_weather_lat_edit"):
    ...
    def api_call(self, action: str, method: str, path: str, payload: dict[str, Any] | None = None, timeout: float = 12.0) -> None:
        def worker() -> None:
            try:
                ...
                if self._recover_backend_for_api_error(action, exc):
                    try:
                        data = self.client.request(method=method, path=path, payload=payload, timeout=timeout)
                        self.api_result.emit(action, True, data)
                        return
    ...
    def _recover_backend_for_api_error(self, action: str, exc: Exception) -> bool:
        if action == "status":
            return False
        text = str(exc).lower()
        recoverable = any(
        ...
        if not recoverable:
            return False
        if not self._backend_recovery_lock.acquire(blocking=False):
            time.sleep(1.5)
            return True
        try:
            subprocess.run(
            ...
            while time.time() < deadline:
                try:
                    self.client.request("GET", "/health", None, timeout=1.0)
                    return True
                except Exception:
                    time.sleep(0.4)
            return False
            ...
            self._backend_recovery_lock.release()
    ...
    def _update_status(self, status: dict[str, Any]) -> None:
        ...
        if hasattr(self, "cfg_brightness_slider") and "brightness_percent" in cfg:
            try:
                brightness = max(0, min(150, int(cfg.get("brightness_percent", 100))))
            except Exception:
                brightness = 100
            if not self.cfg_brightness_slider.isSliderDown() and self.cfg_brightness_slider.value() != brightness:
                self.cfg_brightness_slider.blockSignals(True)
                try:
                    self.cfg_brightness_slider.setValue(brightness)
                finally:
                    self.cfg_brightness_slider.blockSignals(False)
                self._update_lcd_brightness_label(brightness)
    ...
    def apply_config(self) -> None:
        ...
        if hasattr(self, "cfg_brightness_slider"):
            payload.update(self._lcd_config_payload())
        if hasattr(self, "cfg_weather_lat_edit"):
    ...
    def _usb_recovery_script_path(self) -> Path:
        root = Path(__file__).resolve().parent
        candidates = [
            root / "scripts" / "trofeo_usb_power_cycle.sh",
            Path("/usr/share/open-trofeo-lcd/scripts/trofeo_usb_power_cycle.sh"),
        ]
    ...
    def run_usb_recovery(self) -> None:
        script = self._usb_recovery_script_path()
        if not script.exists():
            QMessageBox.warning(
                self,
                "USB recovery",
                f"Nie znaleziono skryptu recovery:\n{script}",
            )
            return
        answer = QMessageBox.question(
            self,
            "USB recovery",
            "Ta operacja zatrzyma backend, wykona reset USB Trofeo LCD i uruchomi backend ponownie.\n\n"
            "Będzie potrzebne sudo w otwartym terminalu. Kontynuować?",
        ...
        command = (
            "echo 'Stopping Open Trofeo LCD backend...'; "
            "systemctl --user stop trofeo-backend.service || true; "
            "echo; echo 'Running Trofeo LCD USB recovery. sudo password may be required.'; "
            f"sudo {shlex.quote(str(script))}; "
            "recovery_status=$?; "
            "echo; echo 'Restarting Open Trofeo LCD backend...'; "
            "systemctl --user restart trofeo-backend.service; "
            "echo; echo \"USB recovery finished with exit code ${recovery_status}. Press Enter to close this terminal.\"; "
            "read"
        )
        terminal_cmds = [
        ...
        for cmd in terminal_cmds:
            ...
            try:
                ...
                self.append_log("[usb-recovery] Terminal opened; sudo may be required.")
                return
            except Exception:
                continue
        QMessageBox.information(
            self,
            "USB recovery",
            "Nie znalazłem emulatora terminala. Uruchom ręcznie:\n\n"
            f"systemctl --user stop trofeo-backend.service\n"
            f"sudo {shlex.quote(str(script))}\n"
            f"systemctl --user restart trofeo-backend.service",
        )
    ...
    def add_game_fps_widget(self) -> None:
        if self.theme_doc_model is None:
            self.reload_designer_from_json()
        if self.theme_doc_model is None:
            return
        ...
        panels.append(
            {
                "id": self._next_item_id("panels", "panel_game_fps"),
                "rect": [1328, 288, 520, 108],
                "radius": 18,
                "fill": [8, 14, 24, 185],
                "opacity": 1.0,
                "z_index": 97,
        ...
        stats.append(
            {
                "id": self._next_item_id("stats", "stat_game_fps"),
                "label": "FPS",
                "source": "game_fps",
                "format": "{value}",
                "x": 1360,
        ...
        stats.append(
            {
                "id": self._next_item_id("stats", "stat_game_name"),
                "label": "",
                "source": "game_name",
                "format": "{value}",
                "x": 1532,
        ...
        stats.append(
            {
                "id": self._next_item_id("stats", "stat_game_frame_time"),
                "label": "",
                "source": "game_frametime_ms",
                "format": "{value}",
                "x": 1534,
                "y": 346,
                "box_width": 132,
        ...
        stats.append(
            {
                "id": self._next_item_id("stats", "stat_game_launcher"),
                "label": "",
        ...
        self.write_designer_to_json()
        self.refresh_designer_element_list()
        self.preview_info_label.setText("Dodano widget Game FPS: FPS, frametime, gra i launcher.")
        self.schedule_preview_theme_doc()
    ...
    def begin_designer_drag(self) -> None:
        ...
        # A render request started before the drag can return after the model
        # has moved. Invalidate it so the preview cannot jump back visually.
        self._preview_request_seq += 1
        self._preview_request_active_seq = self._preview_request_seq
        self.push_designer_history()
...
def main() -> None:
    ...
    app.setDesktopFileName("io.github.abigordiy.open-trofeo-lcd")
    app.setWindowIcon(QIcon.fromTheme("io.github.abigordiy.open-trofeo-lcd"))
